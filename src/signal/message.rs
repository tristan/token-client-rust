...
use super::SignalService::{DataMessage as SignalDataMessage, DataMessage_Flags};
use super::time;
...
impl DataMessage {
    ...
    pub fn deserialize(serialized: &Vec<u8>, timestamp: i64) -> DataMessage {
        let mut dm = SignalDataMessage::new();
        dm.merge_from_bytes(serialized).unwrap();
        DataMessage {
            timestamp: timestamp,
            body: dm.get_body().to_string(),
            end_session: dm.get_flags() == (DataMessage_Flags::END_SESSION as u32),
            expiration_update: dm.get_flags() == (DataMessage_Flags::EXPIRATION_TIMER_UPDATE as u32),
            expires_in_seconds: dm.get_expireTimer()
        }
    }
}
...
impl fmt::Display for TokenMessage {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}@{}: {}", self.sender.to_string(), self.timestamp, self.message)
    }
}
...
mod tests {
    ...
    fn test_deserialize_prekey_message() {
        ...
        // make sure from_base64 does what is expected
        assert_eq!(serialized, exp_serialized);

        // TODO: should probably test decrypting this
        PreKeySignalMessage::deserialize(&serialized).unwrap();
    }
}
